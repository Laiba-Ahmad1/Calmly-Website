...
export async function POST(req: NextRequest) {
  try {
    ...
    // fetch history BEFORE creating today's entry, so it's not comparing against itself
    const previousEntries = await Journal.find({ patientId: user._id })
      .sort({ date: -1 })
      .limit(7)
      .select("feelings reflection date")
      .lean();
  ...
  reflection,
  todos: todos ?? [],
  previousEntries: previousEntries.reverse(),
});
