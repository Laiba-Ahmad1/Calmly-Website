...
import { interpolate } from "@/lib/i18n/dictionaries";
import { formatWeekRange } from "@/lib/format";
import FeedbackEnvelope from "@/components/patient/FeedbackEnvelope";
  ...
  return (
    <div className="relative mx-auto max-w-2xl">
      ...
      {feedback.length === 0 ? (
        ...
        <div className="mt-8 flex flex-col gap-10">
          {feedback.map((f) => {
            const weekLabel = `${interpolate(t("feedback_week"), {
              week: f.weekNumber,
            })} · ${formatWeekRange(f.weekStart, f.weekEnd)}`;

            return (
              <FeedbackEnvelope key={f._id.toString()} weekLabel={weekLabel}>
                <p className="font-body text-xs font-semibold uppercase tracking-wide text-text/50">
                  {weekLabel}
                </p>
